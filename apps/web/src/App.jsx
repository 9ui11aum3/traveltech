...
import { getCities, searchRoutes } from './api'
...
export default function App() {
  ...
  async function search(e) {
    ...
    try {
      ...
      // Inject city coords for the CO2 chart haversine calculation
      const originCity = cities.find(c => c.id === from)
      const destCity   = cities.find(c => c.id === to)
      setResult({ ...data, origin_city: originCity, destination_city: destCity })
      setSelected('fastest')
    } catch (e) {
      setError(e.message)
      setResult(null)
  ...
  function handleOnboardingComplete(p) {
    ...
    if (p.home_city) setFrom(p.home_city)
  }
    ...
    <>
      ...
      <div style={searchPanel}>
        <form style={searchForm} onSubmit={search}>
          ...
          <button type="submit" style={searchBtn} disabled={loading || !from || !to}>
            {loading ? '…' : '🔍 Find Routes'}
          </button>
        </form>
      </div>
      ...
      <div style={container}>
        ...
        {result && (
          <>
            ...
            <RouteMap result={result} selected={selected} cities={cities} />
          </>
        )}
      </div>
...
const field = { display: 'flex', flexDirection: 'column', gap: '.4rem', flex: 1, minWidth: 150 }
...
const selectStyle = {
  ...
  fontSize: '.95rem', color: '#1e293b', background: 'white', outline: 'none',
}
const searchBtn = {
